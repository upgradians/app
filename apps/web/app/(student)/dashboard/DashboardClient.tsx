...
const STATS: StatItem[] = [
  ...
  { icon: "🏆", label: "Contests",   key: "contests_won",      color: "text-yellow-400"     },
  { icon: "🔥", label: "Streak",     key: "streak_days",       color: "text-orange-400", suffix: "d" },
  { icon: "🎯", label: "Missions",   key: "missions_done",     color: "text-purple-400"     },
];
...
export function DashboardClient({ profile, recentSubmissions, topPlayers }: DashboardClientProps) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {/* XP Progress */}
      <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{ delay: .05 }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {STATS.map((s, i) => (
          <motion.div key={s.key} variants={fadeUp} initial="hidden" animate="show" transition={{ delay: .1 + i * .05 }}>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Recent submissions */}
        <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{ delay: .2 }}>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-base font-bold text-[var(--text-1)]">Recent Submissions</h2>
            <Link href="/arena" className="text-xs text-brand font-semibold hover:underline">View all →</Link>
          </div>
          <Card padding="none">
            <div className="divide-y divide-[var(--border)]">
              {recentSubmissions.length === 0 && (
                <div className="p-6 text-center text-[var(--text-3)] text-sm">
                  No submissions yet. <Link href="/arena" className="text-brand">Start coding!</Link>
                </div>
              )}
                ...
                <div key={sub.id} className="flex items-center justify-between px-4 py-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <Link href={`/arena/${sub.challenge?.slug}`}
                        className="text-sm font-semibold text-[var(--text-1)] hover:text-brand truncate block">
                        {sub.challenge?.title ?? "—"}
                      </Link>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {sub.challenge && <DifficultyBadge difficulty={sub.challenge.difficulty as "easy" | "medium" | "hard"} />}
        ...
        {/* Top players */}
        <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{ delay: .25 }}>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-base font-bold text-[var(--text-1)]">Top Coders This Week</h2>
            <Link href="/leaderboard" className="text-xs text-brand font-semibold hover:underline">Full board →</Link>
          </div>
          <Card padding="none">
            <div className="divide-y divide-[var(--border)]">
              {topPlayers.map((p, i) => (
                <div key={p.username} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-bold text-[var(--text-1)] truncate">{p.full_name ?? p.username}</div>
      ...
      <motion.div variants={fadeUp} initial="hidden" animate="show" transition={{ delay: .3 }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { href: "/contests",    icon: "🏆", label: "Join a Contest",    color: "from-yellow-500/15 to-yellow-500/5" },
            { href: "/interview",   icon: "🤖", label: "Practice Interview",color: "from-purple-500/15 to-purple-500/5" },
            { href: "/internships", icon: "🎯", label: "Pick a Mission",    color: "from-emerald-500/15 to-emerald-500/5" },
          ].map(a => (
            <Link key={a.href} href={a.href}>
              <Card hover glow padding="sm" className={`bg-gradient-to-br ${a.color} text-center`}>
