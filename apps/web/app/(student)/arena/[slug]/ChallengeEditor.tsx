...
export function ChallengeEditor({ challenge, submissions, contestMode = false }: Props) {
  const langMeta  = SUPPORTED_LANGUAGES.find(l => l.id === challenge.language) ?? SUPPORTED_LANGUAGES[0];
  ...
  const [result,  setResult]  = useState<{
    status: string; output?: string; runtime?: number; memory?: number; xp_earned?: number;
  } | null>(null);
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      ...
      if (data.status === "accepted") {
        addXP(Number(data.xp_earned ?? challenge.xp_reward ?? 0));
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1.5">
                    ...
                    {contestMode && (
                      <span className="text-xs font-bold text-red-400 bg-red-400/10 border border-red-400/25 px-2 py-0.5 rounded-md animate-pulse">
                        🔴 LIVE
                      </span>
                    )}
                  </div>
                ...
                <div key={sub.id ?? i} className="px-4 py-3 flex items-center justify-between">
                  ...
                  <div className="text-right text-xs text-[var(--text-3)]">
                    ...
                    {Number(sub.xp_earned ?? 0) > 0 && (
                      <span className="ml-2 text-brand font-bold">+{sub.xp_earned} XP</span>
                    )}
                  </div>
                </div>
              ))}
          ...
          {result && (
            <div className={`border-t border-[var(--border)] p-4 text-sm font-mono transition-all ${
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                {result.runtime  != null && <span className="text-xs text-[var(--text-3)]">⏱ {result.runtime} ms</span>}
                {result.memory   != null && <span className="text-xs text-[var(--text-3)]">💾 {Number(result.memory).toFixed(1)} MB</span>}
                {Number(result.xp_earned ?? 0) > 0 && (
                  <span className="text-xs font-bold text-brand">+{result.xp_earned} XP</span>
                )}
              </div>
              {result.output && (
